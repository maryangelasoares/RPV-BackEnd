...
class JobDatabase {
  ...
  createDatabase() {
    ...
    const sql = "CREATE DATABASE IF NOT EXISTS db_job";
    ...
    this.connection.query(sql, (error) => {
      ...
      this.connection.query("USE db_job", (error) => {
        if (error) {
  ...
  createTable() {
    ...
    this.connection.query(sql, (error) => {
      if (error) {
        console.log("Ocorreu um erro ao criar a tabela job..."); // Exibe uma mensagem de erro se a criação da tabela falhar
        console.log(error.message); // Exibe o erro detalhado
        return;
      }
      console.log("Tabela job criada com sucesso..."); // Exibe uma mensagem de sucesso se a criação da tabela for bem-sucedida
    });
  }
}
